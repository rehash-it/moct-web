...
function EditNews({ fetch, news }) {
    ...
    let img = blobCreationFromURL(file + news.image)
    useEffect(() => {
        setState({ ...Disperse(news), image: { value: img, active: false }, imageUrl: file + news.image })
    }, [news])
    const handleFile = e => {
        let image = e.target.files[0]
        if (image.type === 'image/png' || image.type === 'image/jpeg' || image.type === 'image/jpg') {

            setState(s => ({ ...s, image: { value: image, active: true }, imageUrl: URL.createObjectURL(image) }))
            setSave({ process: '', error: '', success: '', imageError: '' })
        }
        else {
            setState(s => ({ ...s, imageUrl: '', image: { value: '', active: false } }))
    ...
    const handleSubmit = async e => {
        ...
        try {
            if (state.image.value !== '') {
                ...
                data.append('image', state.image.value, state.image.active ? state.image.value.name : getFileName(news.image))
